...
export class CalenderComponent implements AfterViewInit, OnInit {
  public currentuserstate:string = localStorage.getItem("state") ;
 ...
 //  public dayStartHourValue: Date = new Date(new Date().setHours(9, 0, 0));
 public dayStartHourValue: Date = new Date(new Date().setHours(8, 0, 0));
  public dayEndHourValue: Date = new Date(new Date().setHours(18,0,0));
 ...
 public workStartHourValue :Date = new Date(new Date().setHours(9,0,0));
  ...
  public generateEvents(test): Record<string, any>[] {
    ...
    for  (let a of test){
      ...
      // this.todayWithPipe = a.starttime;
      ...
      //  EEE MMM d y h:mm:ss z


      console.log("this is starttime", a.starttime)
    }
    ...
    for (const event of overviewEvents) {
      event['StartTime'] = timezone.convert(new Date(event['StartTime']), utcTimezone, currentTimezone);
      event['EndTime'] = timezone.convert(new Date(event['EndTime']), utcTimezone, currentTimezone);
    }
  ...
  public onDayStartHourChange(): void {
    this.scheduleObj.startHour = this.intl.formatDate(this.dayStartHourValue, { skeleton: 'Hm' });
  ...
  public buttonClickActions(e: Event): void {
    ...
    console.log((e.target as HTMLElement).id, 'hetha el ID calendar')
    ...
    if ((e.target as HTMLElement).id === 'add') {
     ...
     console.log("this is startdate ",addObj['StartTime']) ;

      this.rendezVous = {
        description: addObj['Description']  ,starttime :  addObj['StartTime']  , endtime: addObj['EndTime'] ,nomprenom: this.currentuser.nomprenom
      }

        this.rendevouservice.addrendezvous(this.rendezVous,this.id,this.currentuser.id).subscribe((res)=>{
  ...
  public onExportClick(args): void {
    if (args.item.text === 'Excel') {
      ...
      for (const resource of resourceData) {
        const data: Record<string, any>[] = eventCollection.filter((e: Record<string, any>) => e['CalendarI'] === resource['CalendarI']);
  ...
  //     this.selectedResource = args.data['CalendarI'];
  //   }
  //   if (args.type === 'Editor') {
  //     console.log('editor')
  //   }
  // }
  constructor(private fb :FormBuilder , private rendevouservice:RendezvousserviceService, private activatedRoute:ActivatedRoute, private medecinservice:DoctorsserviceService ,
  ...
  getallrendezvous(){
    this.rendevouservice.getallrendezvousbymedecin(this.id).subscribe((data)=>{
      //  this.allrendezvous = data ;
      //  console.log("thhis iss all rendezvous" , data )
      console.log("all rendez vous :",data)
     this.eventSettings = { dataSource: this.generateEvents(data) };
  ...
  todayWithPipe : any ;
  rendezVous:Object ;
  AddApointment:FormGroup ;
  medecin : IDoctor ;
  specialites : any ;
  id:String=this.activatedRoute.snapshot.params['id'] ;
  ngOnInit(): void {
    // this.todayWithPipe = this.pipe.transform(Date.now(), 'EEE MMM d y h:mm:ss zzzz');
    // console.log("this is todaywithpipe" , this.todayWithPipe) ;
    this.getallrendezvous() ;
    this.getmedecinbyid() ;
