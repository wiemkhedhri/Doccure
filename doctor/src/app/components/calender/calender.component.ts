  ...
  public buttonClickActions(e: Event): void {
    ...
    if ((e.target as HTMLElement).id === 'add') {
     ...
     console.log("this is startdate ",addObj['StartTime']) ;
      ...
      this.rendezVous = {
        description: addObj['Description']  ,starttime :  addObj['StartTime']  , endtime: addObj['EndTime'] , nomprenom: this.patient.nomprenom
      }

        this.rendevouservice.addrendezvous(this.rendezVous,this.currentuserid,this.idpatient).subscribe((res)=>{
...
this.getallrendezvous();
  ...
  constructor(private fb :FormBuilder , private rendevouservice:RendezvousserviceService,
    private cd: ChangeDetectorRef , private activatedRoute:ActivatedRoute , private patientservice:PatientserviceService
    ) {

  }
  getallrendezvous(){
    this.rendevouservice.getallrendezvousbymedecin(this.currentuserid).subscribe((data)=>{
...
idpatient = this.activatedRoute.snapshot.params['id'] ;
patient : any ;
  currentuserid = localStorage.getItem('medecinid') ;
  getpatientbyiid(){
    console.log("this is get bby id")
    this.patientservice.getbyid(this.idpatient).subscribe((data )=>{
      this.patient = data ;
      console.log("thisis patiet name", this.patient.nomprenom)
    })
  }
  ngOnInit(): void {
